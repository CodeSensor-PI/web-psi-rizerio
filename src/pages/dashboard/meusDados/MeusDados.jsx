...
const MeusDados = () => {
        ...
        if (!editing) {
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const telefoneEmergencial = telefones.find(t => t.tipo === "EMERGENCIAL");
                        ...
                        await atualizarTelefone(telefonePessoal.id, {
                            ...
                            fkPaciente: {
                                ...
                                fkPlano: {
                                    ...
                                    categoria: usuarioAtual.fkPlano?.categoria || usuarioAtual.fkPlano?.nome,
                                    preco: usuarioAtual.fkPlano?.preco || usuarioAtual.fkPlano?.valor
                                }
                            }
                        });
                    }
    ...
    async function fetchPacienteData() {
        try {
            ...
            const preferencias = await getPreferenciasPorId(localStorage.getItem('idUsuario'));
            ...
            setCep(responseEndereco?.fkEndereco?.cep ? handleCepChange(responseEndereco.fkEndereco.cep) : '');
    ...
    return (
        <>
            ...
            <section className={styles.container_meus_dados}>
                ...
                <button onClick={handleEdit} className={styles.botao_editar}> {editing ? <FaSave /> : <FaPen />} {editing ? 'Salvar' : 'Editar'} </button>
            </section >
        </>
    );
};
